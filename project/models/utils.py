...
class PersonalizedAttentivePooling(nn.Module):
    def __init__(self, query_dim, embedding_dim, user_dim):
        super(PersonalizedAttentivePooling, self).__init__()
        self.query = nn.Linear(user_dim, query_dim)
        self.linear = nn.Linear(embedding_dim, query_dim)    
        

    def forward(self, input_sequence, user_vector):
        query = F.relu(self.query(user_vector)).unsqueeze(dim=-1)
        attention = torch.tanh(self.linear(input_sequence))
        attention = torch.matmul(attention, query).squeeze(dim=-1)
        attention_weights = F.softmax(attention,
                                      dim=1)
        weighted_input = torch.bmm(attention_weights.unsqueeze(dim=1),
                                   input_sequence).squeeze(dim=1)
